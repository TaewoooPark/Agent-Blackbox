...
**最快 —— 无需克隆、无需构建**（需要 Node 20+ 和 [OpenCode](https://opencode.ai)）：

```bash
npx @taewooopark/agent-blackbox up --project /path/to/your/project
```

这一条命令会拉取记录器 + 仪表盘、把记录器插件装进你的项目、启动守护进程，并**在浏览器中自动打开仪表盘**（`http://127.0.0.1:5173/`；加 `--no-open` 可跳过）。然后在该项目里运行你的智能体，地图会实时生成。

<details>
<summary><b>或从源码运行</b>（开发 / 贡献）</summary>
