...
**一条命令 —— 记录每一个 OpenCode 会话**（需要 Node 20+ 和 [OpenCode](https://opencode.ai)）：

```bash
npx @taewooopark/agent-blackbox up
```

这条命令会把记录器装进 OpenCode 的**全局**插件目录（`~/.config/opencode/plugins/`）、启动守护进程，并**打开仪表盘**（`http://127.0.0.1:5173/`；加 `--no-open` 可跳过）。然后照你平时的方式用 OpenCode，地图会实时填充：

```bash
opencode          # 在任意文件夹（终端）
# …或在 OpenCode 桌面应用里打开任意项目
```

无需逐项目设置、无需 `--dir`、无需环境变量 —— 任意会话、任意文件夹，连桌面应用也算。随时用 `npx @taewooopark/agent-blackbox uninstall` 停止记录。

<details>
<summary><b>限定到单个项目，或从源码运行</b></summary>

```bash
# 只记录一个项目（记录器装进 <dir>/.opencode，而非全局）
...
```bash
# 只是观察 —— 启动一次，然后在任意地方用 OpenCode（终端或应用）
npx @taewooopark/agent-blackbox up
opencode   # 在任意文件夹；仪表盘实时填充

# 优化 —— 让免费/本地模型给出定制修复，看右栏
npx @taewooopark/agent-blackbox up --suggest ollama --suggest-model qwen2.5-coder

# 多智能体 —— 在平常的会话里委派，每个子智能体分叉到自己的泳道
opencode "把探索、实现、测试委派给子智能体，然后总结。"
