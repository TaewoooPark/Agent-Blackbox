...
**가장 빠름 — 클론·빌드 없이** (Node 20+ 와 [OpenCode](https://opencode.ai) 필요):

```bash
npx @taewooopark/agent-blackbox up --project /path/to/your/project
```

이 한 줄이 레코더+대시보드를 받아오고, 프로젝트에 레코더 플러그인을 설치하고, 데몬을 시작하고, **대시보드를 브라우저에서 자동으로 엽니다** (`http://127.0.0.1:5173/`; `--no-open`으로 끄기). 그 프로젝트에서 에이전트를 돌리면 맵이 실시간으로 그려집니다.

<details>
<summary><b>또는 소스에서 실행</b> (개발/기여용)</summary>
