...
- **맵 조작** — 팬·줌 캔버스입니다: **휠로 확대/축소**(커서 위치 기준 앵커), **휠 버튼(가운데) 드래그로 상하좌우 패닝**, 빈 공간 드래그로 노드 **다중 선택**, 노드 클릭으로 포커싱. 툴바의 `−` / `%` / `+`로도 줌(**%**는 전체 트리 fit으로 리셋), **Tracing**은 최신 노드를 실시간 추적하거나 뷰를 고정, **Auto layout**은 재정렬.
