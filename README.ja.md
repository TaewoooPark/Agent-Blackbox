...
**1 コマンド — すべての OpenCode セッションを記録**（Node 20+ と [OpenCode](https://opencode.ai) が必要）：

```bash
npx @taewooopark/agent-blackbox up
```

このコマンドがレコーダーを OpenCode の**グローバル**プラグインディレクトリ（`~/.config/opencode/plugins/`）に導入し、デーモンを起動し、**ダッシュボードを開きます**（`http://127.0.0.1:5173/`；`--no-open` で無効化）。あとは普段どおり OpenCode を使えば、マップがライブで埋まっていきます：

```bash
opencode          # 任意のフォルダで（ターミナル）
# …または OpenCode デスクトップアプリで任意のプロジェクトを開く
```

プロジェクトごとの設定も `--dir` も環境変数も不要 — どのセッション・どのフォルダでも、アプリも含めて。記録を止めるには `npx @taewooopark/agent-blackbox uninstall`。

<details>
<summary><b>1 プロジェクトに限定、またはソースから実行</b></summary>

```bash
# 1 プロジェクトだけ記録（レコーダーはグローバルでなく <dir>/.opencode に導入）
...
```bash
# ただ観察 — 一度起動したら、OpenCode を好きな場所で使う（ターミナルでもアプリでも）
npx @taewooopark/agent-blackbox up
opencode   # 任意のフォルダで；ダッシュボードがライブで埋まる

# 最適化 — 無料/ローカルモデルに合わせた修正を出させ、右レーンで確認
npx @taewooopark/agent-blackbox up --suggest ollama --suggest-model qwen2.5-coder

# マルチエージェント — 普段のセッションで委譲すれば各サブエージェントが自分のレーンに分岐
opencode "探索・実装・テストをサブエージェントに委譲してから要約して。"
