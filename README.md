...
- **Map navigation** — it's a pan-and-zoom canvas: the **wheel zooms** (anchored under the cursor) and a **middle-button (wheel) drag pans** in any direction; drag empty space to rubber-band **select** nodes, and click a node to focus it. The toolbar's `−` / `%` / `+` also zoom (the **%** resets to fit the whole tree), **Tracing** follows the newest node live or pins the view, and **Auto layout** re-centers.
